...
class PSARFW_WC_ADMIN_REPORT_HPOS extends WC_Admin_Report {
	...
	public function get_order_report_data( $args = array() ) {
		...
		$args         = apply_filters( 'psarfw_woocommerce_reports_get_order_report_data_args', $args );
		...
		$order_status = apply_filters( 'psarfw_woocommerce_reports_order_statuses', $order_status );
		...
		if ( $filter_range ) {
			$query['where'] .= "
				AND 	posts.date_created_gmt >= '" . esc_sql(get_gmt_from_date( gmdate('Y-m-d H:i:s', $this->start_date) )) . "'
				AND 	posts.date_created_gmt < '" . esc_sql(get_gmt_from_date( gmdate('Y-m-d H:i:s', strtotime( '+1 DAY', $this->end_date )) )) . "'
			";
		}
		...
		$query = apply_filters( 'psarfw_woocommerce_reports_get_order_report_query', $query );
		$query = implode( ' ', $query );
		...
		if ( $debug || $nocache ) {
			...
			$result = apply_filters( 'psarfw_woocommerce_reports_get_order_report_data', $wpdb->$query_type( $query ), $data );
		} else {
